...
Maintaining UI state in the URL lets users refresh, bookmark, or share their exact view. The hook below wraps Next.js useQueryState so any component can toggle a modal while keeping its open/closed state in the query string.
 ...
 *
 * - `parseAsBoolean.withDefault(false)` ensures a strict boolean value.
